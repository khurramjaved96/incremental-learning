            ...
            for class_group in range(0, dataset.classes, args.step_size):
                ...
                for epoch in tqdm(range(0, args.epochs_class)):
                    ...
                    if epoch % args.log_interval == (args.log_interval - 1):
                        ...
                        tScaled = t_classifier.evaluate(my_trainer.model, test_iterator, my_trainer.threshold, False,
                                                        my_trainer.older_classes, args.step_size)
                        tScaledGrad = t_classifier.evaluate(my_trainer.model, test_iterator, my_trainer.threshold2,
                                                            False, my_trainer.older_classes, args.step_size)
                ...
                if args.no_distill:
                    ...
                    tError = tError * float(args.unstructured_size + args.step_size) / (float(args.step_size))
                    testError = t_classifier.evaluate(my_trainer.model_single, test_iterator) * (
                        class_group + args.step_size) / args.step_size
