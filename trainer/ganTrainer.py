...
class trainer():
    ...
    def trainGan(self, G, D):
        ...
        for epoch in range(int(self.args.gan_epochs)):
            ...
            D_like_real = torch.ones(int(self.args.gan_batch_size))
            D_like_fake = torch.zeros(int(self.args.gan_batch_size))
            if self.args.cuda:
            ...
            for image, label in dataIteratorForGan:
                ##################################
                #Train Discriminator
                ##################################
                ...
                G_random_noise = torch.randn((self.args.gan_batch_size, 100))
                ...
                random_labels = (torch.rand(self.args.gan_batch_size, 1)
                                  * len(active_classes_tensor))
