...
class trainer():
    ...
    def trainGan(self, G, D, dataIterator):
        ...
        #TODO Khurram's transform or loader doesnt work with gan, fix it
        ...
        for epoch in range(int(self.args.gan_epochs)):
            ...
            for image, label in train_loader:
                ...
                #labels present in activeClass <- Not anymore
                active_classes_tensor = torch.LongTensor(activeClasses)
                random_labels = (torch.rand(batch_size, 1)
                                 *10).type(
                                 torch.LongTensor).squeeze()
                #TODO Make random_labels of len 10 but restrict them to
                #contain only active classes
